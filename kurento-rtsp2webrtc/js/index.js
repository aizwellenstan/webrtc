...
window.addEventListener('load', function(){
  console = new Console('console', console);
	...
	address.value = 'rtsp://60.251.229.163/user=admin&password=&channel=3&stream=0.sdp';
  var pipeline;
  var webRtcPeer;
    ...
    if(error) return onError(error);
  	...
  	kurentoClient(args.ws_uri, function(error, kurentoClient) {
  		...
  		kurentoClient.create("MediaPipeline", function(error, p) {
  			...
  			pipeline.create("PlayerEndpoint", {uri: "rtsp://60.251.229.163/user=admin&password=&channel=3&stream=0.sdp"}, function(error, playerEndpoint){
  			  if(error) return onError(error);
